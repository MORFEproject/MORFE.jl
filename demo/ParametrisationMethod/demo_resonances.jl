...
res_graph = resonance_set_from_graph_style(
    internal_modes, multiindexset, super_eigenvalues, outer_eigenvalues, 1e-8)
...
ext_cond_local = ConditionNumberEstimateCondition(outer_eigenvalues, 6.0, outer_κ, 1e6,
    (internal_modes + 1):(internal_modes + outer_count))
res_graph_local = resonance_set_from_graph_style(
    internal_modes, multiindexset, super_eigenvalues, ext_cond_local)
for (idx, mi) in enumerate(multiindexset.exponents)
    println("  $mi → [$(format_resonances(res_graph.resonances[:, idx]))]")
end
...
res_cnf = resonance_set_from_complex_normal_form_style(
    internal_modes, multiindexset, super_eigenvalues, target_eigenvalues, 0.2)
for (idx, mi) in enumerate(multiindexset.exponents)
...
res_real = resonance_set_from_real_normal_form_style(
    internal_modes, multiindexset, super_eigenvalues,
    target_eigenvalues, conjugacy_map, 1e-9)
for (idx, mi) in enumerate(multiindexset.exponents)
...
res_local = resonance_set_from_condition_number_estimate(
    internal_modes, multiindexset, super_eigenvalues,
    target_eigenvalues, 6.0, target_κ, 1e6, 1:N_TARGETS)
for (idx, mi) in enumerate(multiindexset.exponents)
