...
function integrate_H!(Fₑ::Vector{ComplexF64}, X::Vector{Float64},
    ...
    for (w, gp) in qr
        ...
        fill!(σᵛ₂₃, 0.0)
        fill!(σᵛ₁₃, 0.0)
        for i in 1:(dim * (dim - 1))
            for j in 1:(dim * (dim - 1))
                σᵛ₁₂[i] += Dᵢⱼₖₗ[j, i]*eᵛ₁₂[j]
                σᵛ₂₃[i] += Dᵢⱼₖₗ[j, i]*eᵛ₂₃[j]
                σᵛ₁₃[i] += Dᵢⱼₖₗ[j, i]*eᵛ₁₃[j]
            end
        ...
        @inbounds for i in 1:nn
            for j in 1:dim
                ...
                sym∇ⁿˡ₂[j + (i - 1) * dim, 3] = (∂N∂x[i, 3]*∇U₂[j, 3])
                sym∇ⁿˡ₃[j + (i - 1) * dim, 3] = (∂N∂x[i, 3]*∇U₃[j, 3])
                sym∇ⁿˡ₁[j + (i - 1) * dim, 4] = (∂N∂x[i, 1]*∇U₁[j, 2] +
                                                 ∂N∂x[i, 2]*∇U₁[j, 1])
                sym∇ⁿˡ₂[j + (i - 1) * dim, 4] = (∂N∂x[i, 1]*∇U₂[j, 2] +
                                                 ∂N∂x[i, 2]*∇U₂[j, 1])
                sym∇ⁿˡ₃[j + (i - 1) * dim, 4] = (∂N∂x[i, 1]*∇U₃[j, 2] +
                                                 ∂N∂x[i, 2]*∇U₃[j, 1])
                sym∇ⁿˡ₁[j + (i - 1) * dim, 5] = (∂N∂x[i, 2]*∇U₁[j, 3] +
                                                 ∂N∂x[i, 3]*∇U₁[j, 2])
                sym∇ⁿˡ₂[j + (i - 1) * dim, 5] = (∂N∂x[i, 2]*∇U₂[j, 3] +
                                                 ∂N∂x[i, 3]*∇U₂[j, 2])
                sym∇ⁿˡ₃[j + (i - 1) * dim, 5] = (∂N∂x[i, 2]*∇U₃[j, 3] +
                                                 ∂N∂x[i, 3]*∇U₃[j, 2])
                sym∇ⁿˡ₁[j + (i - 1) * dim, 6] = (∂N∂x[i, 1]*∇U₁[j, 3] +
                                                 ∂N∂x[i, 3]*∇U₁[j, 1])
                sym∇ⁿˡ₂[j + (i - 1) * dim, 6] = (∂N∂x[i, 1]*∇U₂[j, 3] +
                                                 ∂N∂x[i, 3]*∇U₂[j, 1])
                sym∇ⁿˡ₃[j + (i - 1) * dim, 6] = (∂N∂x[i, 1]*∇U₃[j, 3] +
        ...
        #
        @inbounds for j in 1:(dim * (dim - 1))
            for i in 1:(nn * dim)
                Fₑ[i] += (1.0/6.0) *
                         (sym∇ⁿˡ₁[i, j]*σᵛ₂₃[j] + sym∇ⁿˡ₂[i, j]*σᵛ₁₃[j] +
                          sym∇ⁿˡ₃[i, j]*σᵛ₁₂[j]) * w * Jac_det
            end
        end
        #
    end
