...
function MORFE.Eigenproblems.solve(model::NDOrderModel, solver::Mechanical_Problem_Solver)
    ...
    FK = factorize(K)
    x0 = ones(size(K, 1))
    vals_inv, vecs, _ = eigsolve(x -> FK \ (M * x), x0, nev, :LR;
...
function MORFE.Eigenproblems.solve_left(model::NDOrderModel,
    ...
    for (i, λ) in enumerate(solver.eigenvalues)
        left_eigenvectors[1:FOM, i] =
            -(1 / conj(λ)) * model.linear_terms[1]' *
...
W2, R2 = solve_cohomological_problem(
    model, mset, master_eigenvalues, master_modes, left_eigenmodes, resonance_set;
    master_modes_derivatives = master_modes_derivatives,
)
...
if test_idx !== nothing
    ...
    invariance_C_coeffs, D_steps = precompute_master_column_polynomials(
    ...
    orth_C_coeffs, orth_E_coeffs = precompute_orthogonality_column_polynomials(
    ...
    allocs = @allocated solve_single_monomial!(W2, R2, test_idx, ctx_check, model, ml_cache_check)
