...
function make_ferrite_model(nx, ny, nz; λ, μ, ρ, α_ray, β_ray, max_deg, rom)
    ...
    K = K_full[free, free]; M = M_full[free, free]; C = α_ray * M + β_ray * K
...
mutable struct MechSolver <: AbstractEigensolver
    eig_mat::Union{Nothing, Matrix}; eigenvalues::Union{Nothing, Vector}
...
function MORFE.Eigenproblems.solve(m::NDOrderModel, s::MechSolver)
    ...
    ω2 = real.(ω2[idx]); ϕ = real.(ϕ[:, idx]); ω = sqrt.(ω2); FOM = size(ϕ, 1)
    λ_all = zeros(ComplexF64, 2 * s.nev)
    for i in 1:s.nev
        ξ = 0.5 * (s.α / ω[i] + s.β * ω[i])
        λ_all[2i-1] = ω[i] * (-ξ + sqrt(Complex(1 - ξ^2)) * im); λ_all[2i] = conj(λ_all[2i-1])
    end
    evecs = Matrix{ComplexF64}(undef, 2FOM, 2 * s.nev)
    for i in 1:s.nev
        evecs[1:FOM, 2i-1] .= ϕ[:, i]; evecs[1:FOM, 2i] .= ϕ[:, i]
        evecs[(FOM+1):end, 2i-1] .= λ_all[2i-1] .* ϕ[:, i]
        evecs[(FOM+1):end, 2i]   .= λ_all[2i]   .* ϕ[:, i]
    end
    s.eig_mat = evecs; s.eigenvalues = λ_all; return λ_all, evecs
end
function MORFE.Eigenproblems.solve_left(m::NDOrderModel, s::MechSolver)
    R = s.eig_mat; FOM = size(R, 1) ÷ 2; L = similar(R)
    for i in 1:(length(s.eigenvalues) ÷ 2)
        L[(FOM+1):end, 2i-1] = R[1:FOM, 2i]; L[(FOM+1):end, 2i] = R[1:FOM, 2i-1]
    end
...
const E = 160e3; const ν = 0.22; const ρ_mat = 2.32e-3
...
ep = compute_eigenproblem(model_s;
...
for r in 1:ROM; mmd_s[:, 1, r] .= Y_s[(FOM_s+1):end, r]; end
...
print("    Warm-up ... "); solve_cohomological_problem(solve_args...; solve_kwargs...); println("done.")
