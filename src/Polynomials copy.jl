...
"""
    evaluate(poly::DensePolynomial{T}, vals::Vector{<:Number}) where T

Evaluate a `T` valued polynomial at the given variable values.
Returns a vector of length equal to the number of components.
"""
# ----- Evaluation for scalar polynomials -----
function evaluate(poly::DensePolynomial{T}, vals::Vector{<:Number}) where T<:Number
...
# ----- Evaluation for vector‑valued polynomials -----
function evaluate(poly::DensePolynomial{Vector{T}}, vals::Vector{<:Number}) where T
    @assert nvars(poly) == length(vals) "Number of variables mismatch"
    # Determine the component dimension from the first coefficient.
    # The polynomial is non‑empty because `zero` constructor fills all terms.
    ncomp = length(poly.coeffs[1])
    result = zeros(T, ncomp)
...
# ----- Evaluation of a single component of a vector polynomial -----
function evaluate(poly::DensePolynomial{Vector{T}}, vals::Vector{<:Number}, component::Int) where T
    @assert nvars(poly) == length(vals) "Number of variables mismatch"
    result = zero(T)
    exps = poly.multiindex_set.exponents
    coeffs_vec = poly.coeffs
    for i in 1:size(exps, 2)
        coeff = coeffs_vec[i][component]
        iszero(coeff) && continue
        monom = one(eltype(vals))
        for (j, e) in enumerate(view(exps, :, i))
            monom *= vals[j]^e
        end
        result += coeff * monom
    end
