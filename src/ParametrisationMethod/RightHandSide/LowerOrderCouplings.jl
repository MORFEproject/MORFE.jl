...
@inline function _sum_higher_degree_terms(upper_bound::SVector{NVAR,Int},
    ...
    candidate_idxs = indices_in_box_with_bounded_degree(mset, collect(upper_bound), 2, total_deg_upper)
    isempty(candidate_idxs) && return summation

    # Pre‑compute unit vectors
    unit_vectors = [SVector{NVAR,Int}(ntuple(k -> k == i ? 1 : 0, Val(NVAR))) for i in 1:NVAR]
...
function compute_lower_order_couplings(upper_bound::SVector{NVAR,Int},
                                       parametrisation::Parametrisation{ORD,FOM,NVAR,T},
                                       reduced_dynamics::ReducedDynamics{ROM,NVAR,T}) where {ORD,FOM,NVAR,ROM,T}
