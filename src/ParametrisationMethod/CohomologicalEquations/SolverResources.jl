...
# =============================================================================
# Per-thread solve resources (dense path only)
# =============================================================================

"""
    ThreadedMonoSolveResources{T}

Per-thread mutable state for `solve_cohomological_equations_threaded!`.

Each thread owns independent copies of:
- `buffers`   — system matrix, RHS, external-RHS, and multilinear-result scratch.
- `lo_buffer` — lower-order coupling accumulation vectors (one per ODE order).

Read-only data (`CohomologicalContext`, `ml_cache`, `W`, `R` indices for other
degrees) is shared safely across threads.
"""
struct ThreadedMonoSolveResources{T}
    buffers::CohomologicalBuffers{T}
    lo_buffer::Vector{Vector{T}}     # length ORD; zeroed before each monomial
end

"""
    alloc_thread_resources(FOM, ROM, ORD, n_threads, T) -> Vector{ThreadedMonoSolveResources{T}}

Allocate one `ThreadedMonoSolveResources` per thread.
"""
function alloc_thread_resources(
        FOM::Int, ROM::Int, ORD::Int, n_threads::Int, ::Type{T}
) where {T}
    return [
        ThreadedMonoSolveResources{T}(
            CohomologicalBuffers{T}(FOM, ROM),
            [zeros(T, FOM) for _ in 1:ORD]
        )
