...
#
#      [ L(s_k) | C_{j₁}(s_k)  C_{j₂}(s_k)  … ] * [ W_k; f_{j₁}; f_{j₂}; … ] = RHS_k
#
#  where
#
...
#
#      D_j = Σ_{k=j+1}^{ORD+1} B_k · generalised_right_eigenmodes
#                                        · reduced_dynamics_linear^{k-(j+1)} 
#
#  where `generalised_right_eigenmodes` collects the generalised eigenvectors (master modes
...
"""
	precompute_rhs_columns(B, generalised_right_eigenmodes, reduced_dynamics_linear, ROM)
	-> (C_coeffs, E_coeffs)
...
- `fom_matrices::NTuple{ORD+1}` – linear matrices of the full-order
...
(j goes from ORD down to 1) using one FOM × NVAR working buffer `D`:
	...
	D ← B[ORDP1] * generalised_right_eigenmodes                               (j = ORD)
...
coefficient
	...
	D[:, ·] = Σ_{k=j+1}^{ORDP1} B[k] * generalised_right_eigenmodes * reduced_dynamics_linear^{k-(j+1)}.
...
function precompute_rhs_columns(
	fom_matrices::NTuple{ORDP1, <:AbstractMatrix{T}}, # B[k+1] = Bₖ,  k = 0,…,ORD
	generalised_right_eigenmodes::AbstractMatrix{T},  # FOM × NVAR
	reduced_dynamics_linear::AbstractMatrix{T},       # NVAR × NVAR  (generally upper triangular, or even Jordan form)
	ROM::Int,
	...
	mul!(D, fom_matrices[ORDP1], generalised_right_eigenmodes) # D ← B_{ORD+1} · Y  (step j = ORD)
	for r in 1:ROM
	...
	for j in (ORD-1):-1:1
		...
		mul!(D_tmp, fom_matrices[j+1], generalised_right_eigenmodes, one(T), one(T))  # D_tmp ← D_tmp + B_{j+1} · Y
