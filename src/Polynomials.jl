...
struct DensePolynomial{C, NVAR}
    ...
    function DensePolynomial{C}(
            coeffs::Vector{C}, mset::MultiindexSet{NVAR}) where {C, NVAR}
...
function DensePolynomial(coeffs::Vector{C}, mset::MultiindexSet) where {C}
    DensePolynomial{C}(coeffs, mset)
end
...
function *(s::Number, p::DensePolynomial{C}) where {C}
    DensePolynomial(s .* p.coeffs, p.multiindex_set)
end
