...
# ─────────────────────────────────────────────────────────────────────────────
#
# `eltype` of a DensePolynomial returns T (the scalar element type).  These helpers
# reconstruct the *coefficient* type — what one monomial's worth of data looks like —
# which is what the dictionary-based composition routines below accumulate into.
...
!!! note "Element type"
    The accumulators are typed from `poly`, so a **real** polynomial composed with a
    **complex** `M` cannot store the products back.  Promote the polynomial first, e.g.
    `DensePolynomial(promote_type(eltype(coefficients(p)), TA).(coefficients(p)),
    multiindex_set(p))`.
"""
        ...
        for (key, coeff) in current_dict
            ...
            if e == 0
                ...
                for (k, factor) in exp_dict
                    ...
                    next_dict[new_rest] = get(next_dict, new_rest, _zero_like(scaled_coeff)) +
                                          scaled_coeff
                end
            end
        end
