...
Given variables in the canonical order $z_1,\dots,z_n,\bar z_1,\dots,\bar z_n,w_1,\dots,w_m$, each monomial
...
Multiplying over all $k$ yields a sum of real monomials $x^{\gamma_x}y^{\gamma_y}w^{\gamma_w}$. Coefficients are accumulated over all $\mathbf{m},\mathbf{n}$ satisfying
