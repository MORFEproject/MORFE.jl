...
plot_quad_force.py
==================
Log-log plot of the parametric quadratic-force Taylor-approximation error
vs |θ|, for each truncation order N_θ.

Input:   ../../results/validation/quad_force_errors.csv  (from validate_quadratic_force.jl)
Output:  ../../results/validation/quad_force_errors.png

Analytical expectation (correct assembly):
  N = 1, 2, 3  → slopes 2, 3, 4 in log-log
  N ≥ 4        → error collapses to machine precision  (G_k = 0 for k > 3)

Failure signature (wrong assembly):
  N ≥ 4 lines do NOT collapse to near zero.

Usage:
    python validation/plots/plot_quad_force.py [--no-show]
"""
...
_CSV    = _ROOT / "results" / "validation" / "quad_force_errors.csv"
_OUT    = _ROOT / "results" / "validation" / "quad_force_errors.png"

if not _CSV.exists():
    sys.exit(f"Error: {_CSV} not found.  Run validation/validate_quadratic_force.jl first.")
...
for slope, label in [(2, "slope 2"), (3, "slope 3"), (4, "slope 4")]:
...
ax.set_title("Quadratic force: Taylor approximation error vs θ")
