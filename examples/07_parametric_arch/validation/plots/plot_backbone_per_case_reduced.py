...
==================================
Per-arch-height backbone overlay in reduced domain: reference ROM vs parametric ROM.

For each arch height h₀ for which both reference and parametric backbone data
exist, saves one figure showing (Ω − ω₀) / ω₀ vs. modal amplitude |z|,
...
Uses backbones.csv (amplitude-filtered) as data source so that the reference
curve is limited to physically meaningful r values (where amplitude ≤ A_MAX),
matching the valid range of the reference ROM polynomial expansion.

Input:   ../../results/backbone/backbones.csv   (from validation/backbone.jl)
         ../../results/backbone/metrics.csv
Output:  ../../results/backbone/backbone_<h₀>mm_reduced.png  (one file per arch height)

Usage:
    python validation/plots/plot_backbone_per_case_reduced.py [--no-show]
"""
...
def _title(hr):
    return f"Backbone  —  h₀ = {hr * 1000:.1f} mm  (θ = {hr / H0_L_RATIO - 1:+.2f})  — reduced domain"
...
for hr in h_ratios:
    ...
    fname = f"backbone_{hr * 1000:.1f}mm_reduced.png"
