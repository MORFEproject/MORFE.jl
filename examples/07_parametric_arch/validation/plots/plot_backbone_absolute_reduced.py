...
==================================
Backbone curves in reduced domain — absolute frequency (normalised).

Plots Ω / ω₀_base vs. modal amplitude |z| for every arch height h₀,
...
Output:  ../../results/backbone/backbone_absolute_reduced.png

Usage:
    python validation/plots/plot_backbone_absolute_reduced.py [--no-show]
"""
...
def select(hr, model, z_order):
    tok = int(z_order)
    return df[np.isclose(df.h_ratio, hr) & (df.model == model) &
              (df.z_order == tok)].copy()
...
ax.set_xlabel("Backbone frequency  Ω / ω₀")
ax.set_ylabel("Modal amplitude  |z|")
ax.set_title("Backbone curves — absolute frequency (normalised) — reduced domain")
ax.set_ylim(bottom=0)

fig.tight_layout()
out = OUT_DIR / "backbone_absolute_reduced.png"
