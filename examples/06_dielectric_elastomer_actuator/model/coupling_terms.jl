...
function build_nonlinear_terms(p, fe, bp; forced::Bool, include_cubic_forcing::Bool = true)
	...
	q1! = (res, a1, a2) -> (res .+= ((c2 / 2) * (dot(ℓ0, a1) * dot(gv, a2) +
									 dot(gv, a1) * dot(ℓ0, a2)) -
									cs * dot(ℓ0, a1) * dot(ℓ0, a2)) .* bvec; nothing)
	q2! = (res, a, y) -> (res .+= (c2 * dot(gv, a) * dot(ℓ1, y) -
								   2cs * dot(ℓ0, a) * dot(ℓ1, y)) .* bvec; nothing)
	q3! = (res, a, z) -> (res .+= (c2 * dot(gv, a) * dot(ℓ2, z) -
								   2cs * dot(ℓ0, a) * dot(ℓ2, z)) .* bvec; nothing)
	...
	t1! = (res, a1, a2, a3) -> (res .+= (c3 * dot(ℓ0, a1) * dot(ℓ0, a2) * dot(ℓ0, a3)) .* bvec; nothing)
	t2! = (res, a1, a2, y) -> (res .+= (3c3 * dot(ℓ0, a1) * dot(ℓ0, a2) * dot(ℓ1, y)) .* bvec; nothing)
	t3! = (res, a1, a2, z) -> (res .+= (3c3 * dot(ℓ0, a1) * dot(ℓ0, a2) * dot(ℓ2, z)) .* bvec; nothing)
	t4! = (res, a, y1, y2) -> (res .+= (3c3 * dot(ℓ0, a) * dot(ℓ1, y1) * dot(ℓ1, y2)) .* bvec; nothing)
	t5! = (res, a, y, z) -> (res .+= (6c3 * dot(ℓ0, a) * dot(ℓ1, y) * dot(ℓ2, z)) .* bvec; nothing)
	t6! = (res, a, z1, z2) -> (res .+= (3c3 * dot(ℓ0, a) * dot(ℓ2, z1) * dot(ℓ2, z2)) .* bvec; nothing)
	t7! = (res, y1, y2, y3) -> (res .+= (c3 * dot(ℓ1, y1) * dot(ℓ1, y2) * dot(ℓ1, y3)) .* bvec; nothing)
	t8! = (res, y1, y2, z) -> (res .+= (3c3 * dot(ℓ1, y1) * dot(ℓ1, y2) * dot(ℓ2, z)) .* bvec; nothing)
	t9! = (res, y, z1, z2) -> (res .+= (3c3 * dot(ℓ1, y) * dot(ℓ2, z1) * dot(ℓ2, z2)) .* bvec; nothing)
	t10! = (res, z1, z2, z3) -> (res .+= (c3 * dot(ℓ2, z1) * dot(ℓ2, z2) * dot(ℓ2, z3)) .* bvec; nothing)
	...
	if forced
		...
		if include_cubic_forcing
			f2a! = (res, a1, a2, r) -> (res .+= (cf2 * dot(ℓ0, a1) * dot(ℓ0, a2) * sum(r)) .* bvec; nothing)
			f2b! = (res, a, y, r) -> (res .+= (2cf2 * dot(ℓ0, a) * dot(ℓ1, y) * sum(r)) .* bvec; nothing)
			f2c! = (res, a, z, r) -> (res .+= (2cf2 * dot(ℓ0, a) * dot(ℓ2, z) * sum(r)) .* bvec; nothing)
			f2d! = (res, y1, y2, r) -> (res .+= (cf2 * dot(ℓ1, y1) * dot(ℓ1, y2) * sum(r)) .* bvec; nothing)
			f2e! = (res, y, z, r) -> (res .+= (2cf2 * dot(ℓ1, y) * dot(ℓ2, z) * sum(r)) .* bvec; nothing)
			f2f! = (res, z1, z2, r) -> (res .+= (cf2 * dot(ℓ2, z1) * dot(ℓ2, z2) * sum(r)) .* bvec; nothing)
