...
_js_str(s::AbstractString) = '"' * replace(String(s), '\\' => "\\\\", '"' => "\\\"",
								 '\n' => "\\n") * '"'
...
write_lattice(path, panel::LatticePanel; kwargs...) =
...
function _conditions_js(c::LatticeConditions)
	...
	conds(ps) = "[" * join((string("{name:", _js_str(n), ",mask:[",
...
_unit_svector(α::SVector{N, Int}, i::Int) where {N} =
...
function _spectrum_js(sp::LatticeSpectrum)
	...
	# A member whose immediate divisor falls outside the band: these are precisely the
	# monomials that make the set illegal for `parametrise`, so they get their own mark.
	function breaks_closure(α)
		inband(α) && sum(α) > 1 || return false
		any(1:nvar) do i
			α[i] > 0 && !inband(α - _unit_svector(α, i))
		end
	end
	pts = String[]
	for v in exps
		z = s(v)
		mark = !inband(v) ? "dropped" : (breaks_closure(v) ? "violator" : "kept")
		push!(pts, string("{a:[", join(Vector{Int}(v), ","), "],d:", sum(v),
			",m:", _js_str(mark),
			",v:", _js_num(abs(z)),
			",re:", _js_num(real(z)), ",im:", _js_num(imag(z)), "}"))
	end
	lam = join((string("[", _js_num(real(l)), ",", _js_num(imag(l)), "]")
...
#plane { position:relative; flex:1; min-height:0; min-width:0;
  border:1px solid var(--hair); border-radius:6px; background:#07070b; overflow:hidden; }
...
const VIOLATOR = "#E8A33D";   // breaks downward closure — amber, unused elsewhere
...
function tooltipFor(P, e) {
  ...
  if (e.reps.length === 1) {
	const p = e.reps[0];
	lines.push("α = (" + p.a.join(", ") + ")", "|α| = " + p.d);
	if (p.m === "dropped") lines.push("not in the set");
	if (finite(p.v) && P.valueLabel) lines.push(P.valueLabel + " = " + p.v.toFixed(4));
...
function render3D(P, pts) {
  ...
  const vmax = Math.max(...pts.map(p => finite(p.v) ? p.v : 0), 0);
  // The layer a point belongs to is its third displayed coordinate.
  ...
  function draw() {
	...
	for (const o of q) {
	  ...
	  if (o.p.mark === "dropped") {
		...
		if (o.p.mark === "violator") {   // kept, but a divisor of it was not
		  ctx.strokeStyle = VIOLATOR; ctx.lineWidth = 2.4;
		  ctx.beginPath(); ctx.arc(o.s.x, o.s.y, r + 2.2, 0, 6.2832); ctx.stroke();
		}
	  }
	  o.r = r;
	}
...
function renderPlane(P) {
  ...
  // axes, clipped to the window
  add("line", { x1:sx(view[0]), y1:sy(0), x2:sx(view[1]), y2:sy(0),
	stroke:"#3a3a48", "stroke-width":1 });
  add("line", { x1:sx(0), y1:sy(view[2]), x2:sx(0), y2:sy(view[3]),
	stroke:"#3a3a48", "stroke-width":1 });
  ...
  lbl(sx(view[1]) - 2, sy(0) - 7, "Re", "end");
  lbl(sx(0) + 7, sy(view[3]) + 10, "Im", "start");
  ...
  const vmax = Math.max(...P.pts.map(p => p.v), 0);
  for (const p of P.pts) {
	const drop = p.m === "dropped", bad = p.m === "violator";
	add("circle", { cx:sx(p.re), cy:sy(p.im), r: drop ? 3.4 : 4.8,
	  fill: drop ? "none" : (vmax > 0 ? ramp(p.v/vmax) : COL.kept),
	  stroke: drop ? "#4a4a58" : (bad ? VIOLATOR : "rgba(0,0,0,0.45)"),
	  "stroke-width": drop ? 1.2 : (bad ? 2.6 : 0.8),
	  "stroke-dasharray": drop ? "2 2" : "none" });
  }
...
function renderLegend(P, pts) {
  ...
  const bits = [];
  if (P.valueLabel && pts.some(p => finite(p.v))) {
  ...
  if (seen.has("dropped"))
	bits.push('<span><i style="border:1px dashed #4a4a58"></i>' +
	  (P.lam ? "outside the band" : "removed") + "</span>");
  if (seen.has("violator"))
	bits.push('<span><i style="background:transparent;border:2px solid ' + VIOLATOR +
	  '"></i>divisor outside the band</span>');
  if (P.lam) {
