...
function _spectrum_js(sp::LatticeSpectrum)
    ...
    # Outside the band, yet a divisor of something inside it: exactly the monomials the
    ...
    * { box-sizing:border-box; }
    ...
    function tooltipFor(P, e) {
      ...
      if (e.reps.length === 1) {
    	...
    	if (p.m === "dropped") lines.push("outside the band");
    	if (p.m === "needed") lines.push("outside the band —", "the closure must add it back");
    ...
    function renderLegend(P, pts) {
      ...
      if (seen.has("needed"))
    	bits.push('<span><i style="background:transparent;border:2px solid ' + NEEDED +
    	  '"></i>divisor the closure must add</span>');
      if (P.lam) {
