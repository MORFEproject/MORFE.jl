...
# Read the ROM coefficient from the CSV generated by main.jl.
# The (1,0,1,0) monomial in R gives Im(R₁)/ω₀ = dω/dθ₁ / ω₀.
let csv_path = joinpath(@__DIR__, "..", "results", "data", "R_coefficients.csv")
    if isfile(csv_path)
        lines = readlines(csv_path)
        # Header: exp_1,exp_2,exp_3,exp_4,R1_re,R1_im,...
        # Find ω₀ from the (1,0,0,0) row (R1_im = ω₀ for the undamped eigenvalue iω₀).
        ω₀_rom = nothing
        R1_im_1010 = nothing
        for line in lines[2:end]
            cols = split(line, ',')
            length(cols) >= 7 || continue
            exps = parse.(Int, cols[1:4])
            R1_im = parse(Float64, cols[6])  # imaginary part of mode-1 coefficient (col 6)
            if exps == [1, 0, 0, 0]
                ω₀_rom = R1_im
            elseif exps == [1, 0, 1, 0]
                R1_im_1010 = R1_im
        ...
        if ω₀_rom !== nothing && R1_im_1010 !== nothing
            @printf "ROM polynomial coefficient (1,0,1,0): %+.6f\n" (R1_im_1010 / ω₀_rom)
        else
            println("ROM polynomial coefficient: could not parse R_coefficients.csv")
        end
    else
        println("ROM polynomial coefficient: R_coefficients.csv not found — run main.jl first")
    end
end
