...
const EIGENMODES_DIR = joinpath(OUT_DIR, "eigenmodes")
mkpath(EIGENMODES_DIR)
...
if haskey(d, :all_modes)
	...
	for k in 1:n_modes
		...
		local fname = joinpath(EIGENMODES_DIR, @sprintf("eigenmode_%03d", k))
		...
		println("  → eigenmodes/eigenmode_$(lpad(k,3,'0')).vtu")
	end
else
	...
	vel_im, pres_im = scatter_to_nodes(phi1_im_full, d.cell_dofs, d.cell_connectivity,
		n_nodes, d.n_vel_dofs_per_cell)
	vtk_write(joinpath(EIGENMODES_DIR, "eigenmode_001"), points, cells,
		vel_re, pres_re;
		velocity_Im = vel_im,
...
if haskey(d, :all_modes)
	...
	for k in 1:n_modes
		...
		for (f, θ) in enumerate(thetas)
			...
			local fname = joinpath(mode_dir, @sprintf("mode_%03d_%03d", k, f))
			vtk_grid(fname, points, cells) do vtk
		...
		vtk_save(pvd)
		@printf("  → mode_%03d/mode_%03d.pvd  (%d frames)\n", k, k, N_FRAMES)
	end
end
...
const VORTEX_DIR = joinpath(OUT_DIR, "vortex")
mkpath(VORTEX_DIR)
pvd    = paraview_collection(joinpath(VORTEX_DIR, "vortex"))
thetas = range(0.0, 2π; length = N_FRAMES + 1)[1:N_FRAMES]
...
for (k, θ) in enumerate(thetas)
	...
	local fname = joinpath(VORTEX_DIR, @sprintf("vortex_%03d", k))
	vtk_grid(fname, points, cells) do vtk
...
if haskey(d, :all_modes)
	...
	println("  eigenmodes/eigenmode_001 … eigenmode_$(lpad(n_modes,3,'0')).vtu")
	println("  mode_001/mode_001.pvd … mode_$(lpad(n_modes,3,'0'))/mode_$(lpad(n_modes,3,'0')).pvd  (linear orbit, $N_FRAMES frames each)")
	println("  vortex/vortex.pvd  (nonlinear W, $N_FRAMES frames)")
else
	println("  base_flow.vtu, eigenmodes/eigenmode_001.vtu, vortex/vortex.pvd")
...
println("  • eigenmodes/eigenmode_NNN.vtu     → colour by 'vorticity_Re' or 'vorticity_Im' to see mode shape")
println("  • mode_NNN/mode_NNN.pvd / vortex/vortex.pvd → animate; switch between:")
