...
"""
> assembly_H!(Cp, entry,Ψ₁, Ψ₂, Ψ₃,mesh, U, mult = 1.0)
...
"""
> assembly_G!(Cp, entry,Ψ₁, Ψ₂,mesh, U, mult = 1.0)
...
"""
"""
function assembler_MK!(mesh::Grid, U::Field, K::SparseMatrixCSC{Float64, Int64},
    ...
    for iΩ in mesh.Ω
        ...
        for set in 1:(iΩ.Sen)
            ...
            for e in 1:iΩ.ne[set]
                ...
                dofs!(U, nn, conn, dofs)
                integrate_MK!(
                    Mₑ, Kₑ, X, ρ, Dᵢⱼₖₗ, N, ∂N∂a, ∂N∂x, sym∇, Jac, Jac⁻¹, nn, etype, qr)
                for irow in 1:(nn * dim)
                    if dofs[irow] > 0
                        for jcol in 1:(nn * dim)
                            if dofs[jcol] > 0
