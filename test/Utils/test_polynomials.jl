...
function poly_approx_equal(
        p1::DensePolynomial, p2::DensePolynomial; rtol = 1e-12, atol = 1e-12)
...
@testset "Polynomials module" begin
    ...
    @testset "Constructors from coefficient vector and MultiindexSet" begin
        ...
        @test all(i -> !(i in (idx_20, idx_01)) ? p_dense.coefficients[i] == 0 : true,
            1:length(set))
    end
    ...
    @testset "Error conditions" begin
        ...
        @test_throws Exception polynomial_from_pairs(
            DensePolynomial{Int}, [[1, 0] => 1, [1, 0, 0] => 2])
    end
