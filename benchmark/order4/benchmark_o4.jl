...
function make_ferrite_model(nx, ny, nz; λ, μ, ρ, α_ray, β_ray, max_deg, rom)
    ...
    tq = FerriteGeometricNonlinearity{2}(
        dh, cv, free_to_local, n_free, λ, μ; max_unique_cols = max_uniq)
    tc = FerriteGeometricNonlinearity{3}(
        dh, cv, free_to_local, n_free, λ, μ; max_unique_cols = max_uniq)
...
_replay_all_fem_splits!(
    result_f, model_f, W_c, gs3, cache_f.global_∇W_qp, cache_f.global_Fe_buffers)
...
n_prim = count(
    l -> !any(cache_f.global_fem_splits[l] isa MORFE.MultilinearTerms.FEMGlobalSplit &&
                  isempty(cache_f.global_fem_splits[l].global_unique_cols)
    for l in [l]),
    1:length(mset_f))   # rough count — unused here
