...
B0=[2.0 -1.0; -1.0 2.0]; B2=[1.0 0.0;0.0 1.0]; B1=0.001*B2
...
fails = 0
chk(n,c) = (global fails; c || (fails+=1); @printf("  %-50s %s\n", n, c ? "ok" : "MISMATCH"))
...
chk("unified parametrise ≡ old entry point (W)", Wo.poly.coefficients == Wn.poly.coefficients)
chk("unified parametrise ≡ old entry point (R)", Ro.poly.coefficients == Rn.poly.coefficients)

# MultiindexSet as expansion_order
mset = all_multiindices_up_to(2, 5; min_degree=1)
Wm, Rm = parametrise(model, sd, mset;
    resonance = ResonanceConfig(style=:complex_normal_form, tol=0.05, warn_outer=false))
chk("expansion_order::MultiindexSet ≡ Integer", Wm.poly.coefficients == Wn.poly.coefficients)
...
chk("conjugate symmetry from bundle ≡ explicit literal", Wc.poly.coefficients == Wr.poly.coefficients)
