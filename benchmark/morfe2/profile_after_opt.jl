...
quadratic_term = MultilinearMap(
	(
		res, Ψ₁, Ψ₂) -> Morfe_2_0.assembly_G!(res, Ψ₁, Ψ₂, mesh, U), (2, 0))
...
function MORFE.SpectralDecomposition.eigensolve(model::NthOrderModel, s::MechSolver)
	...
	FK = factorize(K)
	vals_inv, vecs,
	_ = eigsolve(x -> FK \ (M * x), ones(size(K, 1)), s.nev, :LR;
...
eig = spectrum(model,
	...
	sorter! = (args...) -> nothing, normaliser! = (args...) -> nothing)
